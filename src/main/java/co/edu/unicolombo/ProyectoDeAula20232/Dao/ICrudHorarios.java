...
public interface ICrudHorarios extends JpaRepository<Horarios, Integer>{
    ...
    @Query("SELECT h FROM Horarios h WHERE h.estado = 'Activo' AND h.dia = ?1 AND h.actividad.idActividadProgramada = ?2")
    public Horarios verificarHorario(String dia, int idActividad);
}
